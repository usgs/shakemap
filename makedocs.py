...
def main(args):
    ...
    sphinx_cmd = (
        'sphinx-apidoc -o %s -f -e -d 12 -H "%s" -A "%s"'
        " -V %s -T %s shakelib/rupture/gc2.py"
        % (SHAKELIB_API_DIR, SHAKELIB_PACKAGE, AUTHORS, verstr, SHAKELIB_PACKAGE_DIR)
    )
    res, stdout, stderr = get_command_output(sphinx_cmd)
    if not res:
        raise Exception(
...
if __name__ == "__main__":
    desc = (
        "Create API documentation for ShakeMap and optionally post "
