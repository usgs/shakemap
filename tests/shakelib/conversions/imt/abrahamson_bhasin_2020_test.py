...
def test_abrahamsonbhasin2020():
    ...
    sigma = np.array([0.72, 0.72, 0.72, 0.72, 0.72, 0.72, 0.72, 0.72, 0.72, 0.72, 0.72])
    tau = np.array([0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4])
    phi = np.array([0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6])
    sdtypes = [const.StdDev.TOTAL, const.StdDev.INTER_EVENT, const.StdDev.INTRA_EVENT]
    rrup = np.array([2.0, 10.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0, 90.0, 100.0])
    vs30 = np.array(
        [200.0, 180.0, 420.0, 630.0, 740.0, 550.0, 360.0, 270.0, 480.0, 290.0, 600.0]
    )

    pgv, stddevs = ab2020.getPGVandSTDDEVS(psa, [sigma, tau, phi], sdtypes, rrup, vs30)
    # print(repr(pgv))
