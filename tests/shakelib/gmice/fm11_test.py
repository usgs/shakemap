...
def test_fm11():
    ...
    mi_target = np.array(
        [3.68942245, 4.86791195, 6.15942245, 6.90296654, 7.33791195, 7.64651063, 7.88587836])
    np.testing.assert_allclose(mi, mi_target)
    ...
    amps_target = np.array([-6.60298051 ,-5.71050567, -4.81803083, -3.92555598, -2.85458617, -1.87286385, -1.15888397])
    ...
    amps_target = np.array(
        [-3.04725091, -2.06742747, -1.08760402, -0.10778058,  1.06800755, 2.14581334, 2.9296721 ])
    np.testing.assert_allclose(amps, amps_target)
    assert((set(dadm) - dadm_target[PGV()]) == set())

    amps, dadm = gmice.getGMfromMI(mmi_in, SA(0.3), dists=None, mag=6.0)
    amps_target = np.array(
        [-6.18008474, -5.24786405, -4.31564337, -3.38342268, -2.26475786, -1.23931511, -0.49353856])
    np.testing.assert_allclose(amps, amps_target)
    assert((set(dadm) - dadm_target[SA(0.3)]) == set())

    amps, dadm = gmice.getGMfromMI(mmi_in, SA(1.0), dists=dists, mag=8.0)
    amps_target = np.array([-8.14657017, -7.02335793, -5.90014569, -4.77693345, -3.42907876, -2.19354529,-1.2949755])
    assert((set(dadm) - dadm_target[SA(1.0)]) == set())

    amps, dadm = gmice.getGMfromMI(mmi_in, SA(3.0), dists=None, mag=5.0)
    amps_target = np.array([-9.54805824, -8.3967657,  -7.24547315, -6.0941806,  -4.71262955, -3.44620775,-2.52517371])
    np.testing.assert_allclose(amps, amps_target)
    np.testing.assert_allclose(amps, amps_target)
