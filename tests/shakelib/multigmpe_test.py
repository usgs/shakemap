...
def test_basic():
    ...
    mgmpe = MultiGMPE.__from_list__([ASK14, CY14], [0.6, 0.4], imc=const.IMC.RotD50)
    ...
    mgmpe = MultiGMPE.__from_list__(
        [ASK14, C03], [0.6, 0.4], imc=const.IMC.RotD50, default_gmpes_for_site=[ASK14]
    )
    ctx = stuff_context(sctx, rctx, dctx)
    ...
    # Two multi-GMPEs
    mgmpe1 = MultiGMPE.__from_list__([ASK14, CY14], [0.6, 0.4], imc=const.IMC.RotD50)
    mgmpe2 = MultiGMPE.__from_list__(
        [ASK14, C03], [0.6, 0.4], imc=const.IMC.RotD50, default_gmpes_for_site=[ASK14]
    )
    mgmpe = MultiGMPE.__from_list__([mgmpe1, mgmpe2], [0.3, 0.7], imc=const.IMC.RotD50)
    lmean_mgmpe, sd_mgmpe = mgmpe.get_mean_and_stddevs(
        sctx, rctx, dctx, IMT, [const.StdDev.TOTAL]
    )
...
def test_from_config_set_of_sets():
    ...
    lmean, sd = test.get_mean_and_stddevs(sctx, rctx, dctx, IMT, [const.StdDev.TOTAL])
...
def test_nga_w2_m8():
    ...
    gmpes = [BooreEtAl2014()]
    gmpe = MultiGMPE.__from_list__(gmpes, [1.0], imc="Average Horizontal (RotD50)")
...
def test_multigmpe_get_site_factors():
    ...
    gmpe = MultiGMPE.__from_list__(gmpes, [0.5, 0.5], imc="Average Horizontal (RotD50)")
    fs = [gmpe.__get_site_factors__(sctx, rctx, dctx, imt.SA(p)) for p in per]
    fs = np.array(fs).reshape((-1))
    target_fs = np.array(
        [
    ...
    gmpe_m1 = MultiGMPE.__from_list__(gmpes, [1.0], imc="Average Horizontal (RotD50)")
    fs1 = [gmpe_m1.__get_site_factors__(sctx, rctx, dctx, imt.SA(p)) for p in per]
    ...
    gmpe_m2 = MultiGMPE.__from_list__(
        gmpes,
        [0.5, 0.5],
        default_gmpes_for_site=[AtkinsonBoore2006()],
        imc="Average Horizontal (RotD50)",
    )
    fs2 = [gmpe_m2.__get_site_factors__(sctx, rctx, dctx, imt.SA(p)) for p in per]
    ...
    fs1 = [
        gmpe_m1.__get_site_factors__(sctx, rctx, dctx, imt.SA(p), default=False)
        for p in per
    ]
    fs1 = np.array(fs1).reshape((-1))
    fs2 = [
        gmpe_m1.__get_site_factors__(sctx, rctx, dctx, imt.SA(p), default=True)
        for p in per
    ]
    fs2 = np.array(fs2).reshape((-1))
...
def test_point_source_stddev_inflation():
    ...
    lnmu, lnsd = mgmpe.get_mean_and_stddevs(sctx, rctx, dctx, iimt, stddev_types)
    ...
    np.testing.assert_allclose(lnsd[4] ** 2 + lnsd[5] ** 2, lnsd[3] ** 2)
    # Now for the first (with PS inflation)
    np.testing.assert_allclose(lnsd[1] ** 2 + lnsd[2] ** 2, lnsd[0] ** 2)
...
def test_multigmpe_get_mean_stddevs():
    ...
    lnmu, lnsd = mgmpe.get_mean_and_stddevs(sctx, rx, dctx, iimt, stddev_types)
...
def test_multigmpe_exceptions():
    ...
    with pytest.raises(Exception) as a:
        ...
        lnmu, lnsd = mgmpe.get_mean_and_stddevs(testsc, rx, dctx, iimt, stddev_types)
