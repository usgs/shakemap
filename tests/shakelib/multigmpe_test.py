...
def test_basic():
    ...
    mgmpe1 = MultiGMPE.__from_list__(
        [ASK14, CY14], [0.6, 0.4], imc=const.IMC.RotD50)
    mgmpe2 = MultiGMPE.__from_list__(
        [ASK14, C03], [0.6, 0.4], imc=const.IMC.RotD50,
        default_gmpes_for_site=[ASK14])
    mgmpe = MultiGMPE.__from_list__(
...
def test_from_config_set_of_sets():
    ...
    cc = np.corrcoef([lmean_c03 + lamp, lmean_pea11 + lamp])
    lmean_c03[dctx.rjb > 500] = lmean_pea11[dctx.rjb > 500]
    cc = ((wts * wts.T) * cc).reshape((2, 2, 1))
    sdlist = [lsd_c03.reshape((1, 1, -1)),
              lsd_pea11.reshape((1, 1, -1))]
    sdstack = np.hstack(sdlist)
    wcov = (sdstack * np.transpose(sdstack, axes=(1, 0, 2))) * cc
    lnsd_scr = np.sqrt(wcov.sum((0, 1)))
    lnsd_scr[dctx.rjb > 500] = lsd_pea11[dctx.rjb > 500]

    wts = np.array([0.8, 0.2]).reshape((1, -1))
    cc = np.corrcoef([lmean_ask14, lmean_scr])
    cc = ((wts * wts.T) * cc).reshape((2, 2, 1))
    sdlist = [lsd_ask14[0].reshape((1, 1, -1)),
              lnsd_scr.reshape((1, 1, -1))]
    sdstack = np.hstack(sdlist)
    wcov = (sdstack * np.transpose(sdstack, axes=(1, 0, 2))) * cc
...
def test_nga_w2_m8():
    ...
    gmpes = [BooreEtAl2014()]
    gmpe = MultiGMPE.__from_list__(gmpes, [1.0],
...
def test_multigmpe_get_site_factors():
    ...
    fs = [gmpe.__get_site_factors__(sctx, rctx, dctx, imt.SA(p)) for p in per]
    ...
    gmpe = MultiGMPE.__from_list__(gmpes, [0.5, 0.5],
                               imc='Average Horizontal (RotD50)')
    fs = [gmpe.__get_site_factors__(sctx, rctx, dctx, imt.SA(p)) for p in per]
    fs = np.array(fs).reshape((-1))
    target_fs = np.array(
        [0.43344744,  0.42524777,  0.41704811,  0.4065989,  0.37570633,
         0.3406088 ,  0.30111541,  0.27347438,  0.28337946,  0.32572201,
    ...
    gmpe_m1 = MultiGMPE.__from_list__(
        gmpes, [1.0], imc='Average Horizontal (RotD50)')
    fs1 = [gmpe_m1.__get_site_factors__(sctx, rctx, dctx, imt.SA(p))
           for p in per]
    fs1 = np.array(fs1).reshape((-1))

    gmpes = [Campbell2003(), AtkinsonBoore2006()]
    gmpe_m2 = MultiGMPE.__from_list__(
        gmpes, [0.5, 0.5], default_gmpes_for_site=[AtkinsonBoore2006()],
        imc='Average Horizontal (RotD50)')
    fs2 = [gmpe_m2.__get_site_factors__(sctx, rctx, dctx, imt.SA(p))
           for p in per]
    fs2 = np.array(fs2).reshape((-1))
    np.testing.assert_allclose(fs1, fs2)

    # --------------------------------------------------------------------------
    # Check that __get_site_factors__ gives the same thing as calling
    ...
    for i in range(len(per)):
        lmean[i], lsd = gmpe_m2.get_mean_and_stddevs(
            ...
            list(gmpe_m2.DEFINED_FOR_STANDARD_DEVIATION_TYPES))
    ...
    fs1 = np.array(fs1).reshape((-1))
    fs2 = [gmpe_m1.__get_site_factors__(sctx, rctx, dctx, imt.SA(p),
...
def test_multigmpe_get_mean_stddevs():
    ...
    lnmud = np.array(
        [[3.59539686,  3.7081893,  3.73286788,  3.78790331,  3.82294539,
          3.86208006,  3.86485444],
         [3.63362187,  3.70523546,  3.76307053,  3.81236243,  3.85884022,
            3.87708871,  3.87363109],
            [3.66714334,  3.7312258,  3.78715775,  3.8263537,  3.89057658,
             3.92954219,  3.93554182],
            [3.67849601,  3.74501608,  3.8009842,  3.8940762,  3.88294242,
             3.87998622,  3.86291218],
            [3.77274389,  3.76610057,  3.83532648,  3.84898303,  3.85213729,
             3.86208438,  3.85244256],
            [3.73608007,  3.77733193,  3.7165333,  3.7566158,  3.78705836,
             3.80706509,  3.80721106],
            [3.67516071,  3.64519877,  3.63887338,  3.67040334,  3.70229907,
             3.72571416,  3.72970474]])

    lnsdd = np.array(
        [[0.59855488, 0.59764991, 0.5984806, 0.59842238, 0.59837916,
          0.59841832, 0.59851621],
         [0.59851021, 0.5984284, 0.59843465, 0.59832939, 0.59818647,
          0.59824473, 0.59834046],
         [0.59852507, 0.59850983, 0.59843657, 0.59831508, 0.59783908,
          0.59578413, 0.59455052],
         [0.59861813, 0.59860025, 0.59849002, 0.59710638, 0.59812893,
          0.59821826, 0.59836214],
         [0.59230603, 0.59778446, 0.5962753, 0.59797146, 0.59839202,
          0.59837343, 0.59839012],
         [0.59144929, 0.5925885, 0.5984268, 0.59855357, 0.59848017,
          0.59843716, 0.59842358],
         [0.59099891, 0.59717933, 0.59860769, 0.59862927, 0.59855412,
          0.59849285, 0.59845718]])
    ...
    lnmud = np.array(
        [[ 3.79370715,  3.91945398,  3.92735788,  3.9854085 ,  4.0251856 ,
           4.02142327,  3.98239189],
         [ 3.83068931,  3.9011462 ,  3.95898835,  4.01371384,  4.04401086,
           4.01895291,  3.97862   ],
         [ 3.86221708,  3.9251345 ,  3.98438864,  4.02991453,  4.06151553,
           4.08259057,  4.07006001],
         [ 3.8710358 ,  3.93730402,  3.99874426,  4.09621909,  4.02718558,
           3.98748004,  3.94313303],
         [ 4.03628379,  3.97433485,  4.06119504,  4.02547659,  3.98200584,
           3.95763481,  3.92389507],
         [ 4.00982228,  4.03787197,  3.912263  ,  3.91507221,  3.9182129 ,
           3.90953284,  3.89042184],
         [ 3.95680584,  3.8638172 ,  3.819042  ,  3.82955957,  3.8398903 ,
           3.84162714,  3.8342373 ]])
    lnsdd = np.array(
        [[ 0.58648564,  0.58648564,  0.58648564,  0.58648564,  0.58648564,
           0.58648564,  0.58648564],
         [ 0.58648564,  0.58648564,  0.58648564,  0.58648564,  0.58648564,
           0.58648564,  0.58648564],
         [ 0.58648564,  0.58648564,  0.58648564,  0.58648564,  0.58648564,
           0.58648564,  0.58648564],
    ...
    lnmud = np.array(
      [[ 3.47869426,  3.62888139,  3.77836798,  3.94365287,  4.07630888,
         4.07551256,  3.96146022],
       [ 3.54822378,  3.69868467,  3.86140403,  4.0201271 ,  4.10007582,
         4.02221208,  3.90868966],
       [ 3.62086858,  3.77802398,  3.94361313,  4.0773018 ,  4.08147362,
         3.97557848,  3.87183821],
       [ 3.6579239 ,  3.8320937 ,  4.00705184,  4.10780181,  4.02385488,
         3.91031541,  3.80752539],
       [ 3.64474435,  3.78052235,  3.92400668,  3.97931146,  3.92993467,
         3.85132698,  3.76030434],
       [ 3.55117   ,  3.66101382,  3.72712467,  3.76126181,  3.75722937,
         3.72819983,  3.6794982 ],
       [ 3.43059761,  3.49239825,  3.53949689,  3.56649963,  3.57845941,
         3.57429337,  3.55446146]])

    lnsdd = np.array(
      [[ 0.65181511,  0.65181511,  0.65181511,  0.65181511,  0.65181511,
         0.65181511,  0.65181511],
       [ 0.65181511,  0.65181511,  0.65181511,  0.65181511,  0.65181511,
         0.65181511,  0.65181511],
       [ 0.65181511,  0.65181511,  0.65181511,  0.65181511,  0.65181511,
         0.65181511,  0.65181511],
    ...
    lnsdd_inter = np.array(
      [[ 0.30024866,  0.30024866,  0.30024866,  0.30024866,  0.30024866,
         0.30024866,  0.30024866],
       [ 0.30024866,  0.30024866,  0.30024866,  0.30024866,  0.30024866,
         0.30024866,  0.30024866],
       [ 0.30024866,  0.30024866,  0.30024866,  0.30024866,  0.30024866,
         0.30024866,  0.30024866],
    ...
    lnsdd_intra = np.array(
      [[ 0.57923543,  0.57923543,  0.57923543,  0.57923543,  0.57923543,
         0.57923543,  0.57923543],
       [ 0.57923543,  0.57923543,  0.57923543,  0.57923543,  0.57923543,
         0.57923543,  0.57923543],
       [ 0.57923543,  0.57923543,  0.57923543,  0.57923543,  0.57923543,
         0.57923543,  0.57923543],
