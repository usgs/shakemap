...
# local imports
from shakemap.c.clib import (geodetic_distance_fast,
...
def test_distances():
    ...
    lons1_radians = np.radians(lons1)
    lats1_radians = np.radians(lats1)
    W = -105.0
    E = -104.0
    S = 66.0
    N = 67.0
    ...
    lons2_radians = np.radians(lons2)
    lats2_radians = np.radians(lats2)
    d1 = np.empty((np.size(lons2), np.size(lons1)))

    geodetic_distance_fast(lons1_radians, lats1_radians,
                           lons2_radians, lats2_radians, d1)
    ...
    geodetic_distance_haversine(lons1_radians, lats1_radians,
                                lons2_radians, lats2_radians, d2)
