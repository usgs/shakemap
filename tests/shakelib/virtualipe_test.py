...
def test_virtualipe():
    ...
    mmi_variable_vs30_intra, mmi_sd_variable_vs30_intra = ipe.get_mean_and_stddevs(
        sx, rx, dx, MMI(), sd_types
    )
    ...
    savefile = os.path.abspath(
        os.path.join(
            homedir, "virtualipe_data", "Calexico", "virtualipe_test", "savefile.npz"
        )
    )
    ...
    mmi_fd, mmi_fdsd = ipe.get_mean_and_stddevs(sx, rx, dx, MMI(), sd_types, fd=0)
