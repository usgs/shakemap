...
def test_assemble_augment():
    ...
    set_files(
        event_path,
        {"event.xml": "event.xml", "model_select.conf.bad0": "model_select.conf"},
    )
    try:
        amod = AssembleModule("nc72282711", comment="Test comment.")
    ...
    set_files(
        event_path,
        {"event.xml": "event.xml", "model_select.conf.bad1": "model_select.conf"},
    )
    try:
        amod = AssembleModule("nc72282711", comment="Test comment.")
        with pytest.raises(RuntimeError):
    ...
    set_files(
        event_path,
        {"event.xml": "event.xml", "model_select.conf.bad2": "model_select.conf"},
    )
    try:
        amod = AssembleModule("nc72282711", comment="Test comment.")
        with pytest.raises(FileNotFoundError):
    ...
    #
    set_files(event_path, {"event.xml": "event.xml", "model.conf.cz": "model.conf"})
    try:
        assemble = AssembleModule("nc72282711", comment="Test comment.")
        assemble.execute()
        augment = AugmentModule("nc72282711", comment="Test comment.")
...
def test_assemble_augment_command_line():
    ...
    data_file = os.path.join(
        datapath, "integration_test_0001", "current", "shake_data.hdf"
    )
    if os.path.isfile(data_file):
        os.remove(data_file)
