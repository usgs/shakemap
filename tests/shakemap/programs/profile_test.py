...
def test_profile():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        #
        cp = subprocess.run(
            [program, "-f", pfile, "-c", "test_profile1", "-a", "-n"], shell=False
        )
        ...
        cp = subprocess.run(
            [program, "-f", pfile, "-c", "test_profile2", "-a", "-n"], shell=False
        )
        assert not cp.returncode
        cp = subprocess.run(
            [program, "-f", pfile, "-c", "test_profile3", "-a", "-n"], shell=False
        )
        assert not cp.returncode
        ...
        cp = subprocess.run(
            [program, "-f", pfile, "-d", "not_a_profile", "-a"], shell=False
        )
        assert cp.returncode
        # Delete a profile that we're not set to (should succeed)
        cp = subprocess.run(
            [program, "-f", pfile, "-d", "test_profile2", "-a"], shell=False
        )
        assert not cp.returncode
        ...
        cp = subprocess.run(
            [program, "-f", pfile, "-d", "test_profile1", "-a"], shell=False
        )
        assert not cp.returncode
        cp = subprocess.run(
            [program, "-f", pfile, "-d", "test_profile3", "-a"], shell=False
        )
        assert not cp.returncode
        # Create a profile and accept the defaults (should succeed)
        # Making the 'test' profile, so it will copy Northridge
        op = subprocess.Popen(
            [program, "-f", pfile, "-c", "test", "-n"],
        ...
        if run_mkdir_tests:
            op = subprocess.Popen(
                [program, "-f", pfile, "-c", "junk_profile", "-n"],
                stdin=subprocess.PIPE,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
                shell=False,
