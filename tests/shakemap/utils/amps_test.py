...
def test_amps():
    try:
        ...
        event = {
            ...
            "netid": "ci",
            "network": "",
            "time": datetime(2018, 3, 7, 18, 5, 0).strftime(constants.TIMEFMT),
            "lat": 35.487,
            "lon": -120.027,
            "depth": 8.0,
        ...
        xmlfile = os.path.join(
            homedir, "..", "..", "data", "ampdata", "USR_100416_20180307_180450.xml"
        )
        ...
        assert info["station_min"] == datetime(
            2018, 3, 7, 18, 4, 49, tzinfo=timezone.utc
        )
        assert info["station_max"] == datetime(
            2018, 3, 7, 18, 4, 49, tzinfo=timezone.utc
        )
        ...
        # various edge cases.
        xmlfile2 = os.path.join(
            homedir, "..", "..", "data", "ampdata", "USR_100416_20180307_180450_2.xml"
        )
        xmlfile3 = os.path.join(
            homedir, "..", "..", "data", "ampdata", "USR_100416_20180307_180450_3.xml"
        )
        xmlfile4 = os.path.join(
            homedir, "..", "..", "data", "ampdata", "USR_100416_20180307_180450_4.xml"
        )
        xmlfile5 = os.path.join(
            homedir, "..", "..", "data", "ampdata", "USR_100416_20180307_180450_5.xml"
        )
        xmlfile6 = os.path.join(
            homedir, "..", "..", "data", "ampdata", "USR_100416_20180307_180450_6.xml"
        )
        handler = AmplitudeHandler(install_path, data_path)
        handler.insertEvent(event)
        handler.insertEvent(event2)
        ...
        xmlfile = os.path.join(
            homedir, "..", "..", "data", "ampdata", "TA109C_BH..2018_095_193003x.xml"
        )
        handler.insertAmps(xmlfile)
        info = handler.getStats()
