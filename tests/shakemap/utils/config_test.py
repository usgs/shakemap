...
def test_config():
    ...
    c1 = ConfigObj(os.path.join(mydatapath, "model.conf"), configspec=myspec)
    c2 = ConfigObj(os.path.join(mydatapath, "modules.conf"), configspec=myspec)
    c3 = ConfigObj(os.path.join(mydatapath, "gmpe_sets.conf"), configspec=myspec)
    c4 = ConfigObj(os.path.join(mydatapath, "northridge_model.conf"), configspec=myspec)
    c5 = ConfigObj(os.path.join(mydatapath, "products.conf"), configspec=myprodspec)
    c1.merge(c2)
    c1.merge(c3)
    c1.merge(c4)
    c1.merge(c5)
    ...
    ctest = ConfigObj()
    ctest["profiles"] = {
        "prof1": {"data_path": "/xyz/zzsx/zz", "install_path": "/xyz/zzsx/zz"},
        "prof2": {"data_path": data_dir, "install_path": install_dir},
    }
    ...
    res = config.nanfloat_list(["-20.0", "-Inf", "NaN", "nan", "Inf", "Nan"], 0)
    assert isinstance(res, list)
    nan = float("nan")
    inf = float("inf")
    minf = float("-inf")
    ...
    with pytest.raises(ValidateError):
        res = config.nanfloat_list(
            ["-20.0", "-Inf", "NaN", "nan", "Inf", "Nan", "thing"], 0
        )
