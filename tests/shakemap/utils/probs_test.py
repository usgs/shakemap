...
def test_probs():
    ...
    np.testing.assert_allclose(gmmodel["weightlist"], [0.191049, 0.808951], rtol=1e-05)
    ...
    np.testing.assert_allclose(
        gmmodel["weightlist"], np.array([0.191049, 0.32369088, 0.48526013]), rtol=1e-05
    )
    ...
    # so m_{int|abs(dz)} = 0.15
    np.testing.assert_allclose(
        gmmodel["weightlist"], np.array([0.191049, 0.808951]), rtol=1e-05
    )
    assert gmmodel["gmpelist"][0] == "active_crustal_deep"
