...
def test_get_config():
    ...
    assert len(set(config["network_delays"].keys()) ^ set(["ci", "nc", "pn"])) == 0
...
def test_event_queue():
    ...
    assert len(events) == 0
    eq.insertRunningEvent("firstevent", ["This", "is", "the", "first", "event"])
    eq.insertRunningEvent("secondevent", ["This", "is", "the", "second", "event"])
    events = eq.getRunningEvents()
    assert len(events) == 2
    assert events.index(("firstevent", ["This", "is", "the", "first", "event"])) >= 0
    assert events.index(("secondevent", ["This", "is", "the", "second", "event"])) >= 0
...
def test_queue():
    ...
    assert qq.magnitudeTooSmall({"mag": 2.0, "lon": -118.25, "lat": 34.0}) is True
    assert qq.magnitudeTooSmall({"mag": 3.6, "lon": -118.25, "lat": 34.0}) is False
    assert qq.magnitudeTooSmall({"mag": 2.0, "lon": -119.25, "lat": 35.0}) is True
    assert qq.magnitudeTooSmall({"mag": 3.9, "lon": -119.25, "lat": 34.6}) is False
    assert qq.magnitudeTooSmall({"mag": 2.0, "lon": -129.25, "lat": 39.0}) is True
    assert qq.magnitudeTooSmall({"mag": 4.1, "lon": -129.25, "lat": 39.0}) is False
    ...
    qq.shake_cmds = shlex.split(
        'associate dyfi select assemble -c "Autorun" ' "model mapping"
    )
    dt = datetime.utcnow()
    event = {
        "id": "test_event",
